...
<!--login-->
	<?php 
		if(isset($_POST['login']))
		{
			...
			$sq="SELECT * FROM customer WHERE c_email='$c_email' AND c_password='$c_password'";
			...
			else if(!preg_match("/^.*(?=.{8,}).*$/",$c_password))
			{
				$error_msg['c_password']= "*Password must be 8 CHARACTER ";
			}
			...
			{
				echo '<script>alert("Invalid UserEmail and UserPassword")</script>';
